...
def run_scientific_benchmark():
    ...
    print(f"{'Taille':<11} | {'Mode':<4} | {'Mo/s':<8} | {'± Std':<7} | {'IPC':<5} | {'± Std':<6} | {'LLC Misses':<12} | {'± Std':<10}")
    print("-" * 85)
    ...
    for size in sizes:
        ...
        for mode in modes:
            ...
            for _ in range(iterations):
                cmd = ["perf", "stat", "-x", ",", "-e", events, sys.executable, __file__, "--run", str(size), "--mode", mode]
            ...
            print(f"{size:<11} | {mode:<4} | {avg_bw:<8.1f} | {std_bw:<7.1f} | {avg_ipc:<5.2f} | {std_ipc:<6.3f} | {avg_llc:<12.0f} | {std_llc:<10.0f}")

    csv_file = "cache_benchmark_results_std.csv"
