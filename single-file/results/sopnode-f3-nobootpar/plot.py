...
for m in membinds:
    for t in threads:
        ...
        plt.title(f"Temps D'exec GROUPBY Streaming - {t} Thread(s) | {mode_str} (m{m})", fontsize=14, fontweight='bold')
...
for m in membinds:
    ...
    plt.title(f"Comparaison Multi-thread GROUPBY Streaming | {mode_str} (m{m})", fontsize=14, fontweight='bold')
    plt.xlabel("Taille du DataFrame (Go)", fontsize=12)
    plt.ylabel("Temps d'exécution (Secondes)", fontsize=12)
