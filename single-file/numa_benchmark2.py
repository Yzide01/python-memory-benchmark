...
# ==========================================
# GESTION DES SIGNAUX & PERF (Zéro Bruit)
# ==========================================
perf_process = None
worker_pid = None
perf_stderr_output = ""

# Utilisation de cache-misses (compatible f1 et f3)
...
# ==========================================
# 1. LE WORKER (Polars Cache-Aware + Lazy)
# ==========================================
def worker(op, n_elements, streaming):
    ...
    if streaming:
        res = query.collect(streaming=True)
    else:
        res = query.collect(streaming=False)
...
def main():
    ...
    parser.add_argument("--op", type=str, choices=["join", "grpby"], default="join")
    ...
    # --- NOUVELLE OPTION POUR LE STREAMING ---
    ...
    for n in sizes:
        ...
        if args.mode == "local":
            cmd = ["taskset", "-c", "0", sys.executable, __file__, "--worker", "--op", args.op, "--elements", str(n)]
        else:
            cmd = ["numactl", f"--cpubind={args.cpubind}", f"--membind={args.membind}", sys.executable, __file__, "--worker", "--op", args.op, "--elements", str(n)]
    ...
    csv_filename = f"benchmark_polars_{args.op}_{args.mode}_{stream_suffix}.csv"
