...
def main():
    parser = argparse.ArgumentParser(description="NUMA Analytical Plotter (Clean Axes with Masking)")
    ...
    main_title = (
        f"NUMA Analysis | Machine: {hostname} | Operation: {args.op.upper()}\n"
    ...
    plot_configs = [
        ...
        (ax_miss, "Cache_Misses", "L3 Cache Misses (LLC Misses)", "L3 Misses", "#d62728"),
        (ax_ipc, "IPC", "Instructions Per Cycle (IPC)", "Instructions / Cycle", "#2ca02c"),
        (ax_ref, "Cache_Refs", "L3 Cache References (LLC Refs)", "L3 Accesses", "#1f77b4"),
    ...
    # 4. Save output
    output_filename = f"plot_clean_{args.op}_c{args.cpubind}m{args.membind}.png"
