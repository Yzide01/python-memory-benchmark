...
def compute_join(n_elements):
    table_a = np.arange(n_elements, dtype=np.uint32)
    table_b = np.arange(n_elements, dtype=np.uint32)
    result = np.isin(table_b, table_a)
    sys.exit(0)
...
def run_numa(n, iters):
    cmd = f"numactl --cpunodebind=0 --membind=0 perf stat -e instructions,cycles,LLC-load-misses python3 wrapper-sim-join-numa.py --run {n}"
    data = {"Instructions": [], "Cycles": [], "IPC": [], "LLC_misses": [], "lat_ns": []}
    print(f"Lancement Join NUMA ({n} éléments, {iters} itérations)...")
