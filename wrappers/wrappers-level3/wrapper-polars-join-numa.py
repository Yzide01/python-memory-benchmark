...
def compute_join(n_elements):
    df_a = pl.DataFrame({"key": np.arange(n_elements, dtype=np.uint32), "val": np.random.rand(n_elements)})
    df_b = pl.DataFrame({"key": np.arange(n_elements, dtype=np.uint32), "val": np.random.rand(n_elements)})
    result = df_a.join(df_b, on="key")
    sys.exit(0)
...
def run_numa(n, iters):
    cmd = f"numactl --cpunodebind=0 --membind=0 perf stat -e instructions,cycles,LLC-load-misses python3 wrapper-polars-join-numa.py --run {n}"
    data = {"Instructions": [], "Cycles": [], "IPC": [], "LLC_misses": [], "lat_ns": []}
    print(f"Lancement Polars Join NUMA ({n} éléments)...")
    ...
    print_table(data, "Polars Join NUMA")
...
if __name__ == "__main__":
    ...
    if args.run: compute_join(args.run)
