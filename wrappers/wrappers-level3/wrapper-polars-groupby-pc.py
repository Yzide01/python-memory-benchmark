...
# 1. Force l'utilisation d'un seul thread pour des mesures déterministes
...
def compute_groupby(n_elements):
    ...
    # Exécution de l'opération de group-by
...
def run_benchmark(n, iters):
    cmd = f"perf stat -e instructions,cycles,LLC-load-misses python3 wrapper-polars-groupby-pc.py --run {n}"
    data = {"Instructions": [], "Cycles": [], "IPC": [], "LLC_misses": [], "lat_ns": []}
    
    print(f"Lancement Polars GroupBy PC ({n} éléments)...")
    for _ in range(iters):
        proc = subprocess.run(cmd, shell=True, capture_output=True, text=True)
        # Capture des métriques via regex
    ...
    print_table(data, "Polars GroupBy PC")
